...
let exportMethods = {
  addUser (username, password) {
    ...
    if (!username || typeof (username) !== 'string') { return Promise.reject('Please provide a valid userName') }
    if (!password || typeof (password) !== 'string') { return Promise.reject('Please provide a valid password') }
  ...
  getUserByUserName (username) {
    if (!username) { return Promise.reject('Please provide a valid username.') }
    return users().then((userCollection) => {
  ...
  getUserById (userId) {
    if (!userId) { return Promise.reject('Please provide a valid userId.') }
  ...
  updateListenHistory (userName, trackId) {
    if (!userName) { return Promise.reject('Please log in first') }

    return users().then((userCollection) => {
      return userCollection.findOne({username: userName}).then((user) => {
        let array = user.listenHistory

        if (array.length > 10) { array.splice(0, 1) } else					{ array.push(trackId) }
  ...
  updateFavorite (userName, trackId) {
    ...
    return users().then((userCollection) => {
      return userCollection.findOne({username: userName}).then((user) => {
        ...
        if (index > -1) { array.splice(index, 1) } else					{ array.push(trackId) }
